...
| `f` / `F`                           | Flash 快速跳转 / Treesitter 结构跳转                       |
...
保存和恢复会话时会保留 Tab、Window 布局及自定义 Tab 名称，并忽略目录参数及隐藏的目录 Buffer，避免通过目录启动 Neovim 后在 Buffer 列表中残留一个会触发文件浏览器的条目。保存会话时还会排除空白窗口，防止 Snacks 文件浏览器被保存为空 Buffer 窗口，并单独记录文件浏览器的开启和焦点状态：退出时文件浏览器处于关闭状态则不会恢复，处于开启状态则在恢复会话后重新打开。会话恢复后，保存的终端窗口会重新创建为 Snacks 终端，而不是保留为 Neovim 原生终端。保存完成后会恢复原有的 `sessionoptions`。
