...
map("n", "<S-h>", "<cmd>tabprevious<CR>", { desc = "上一个 Tab" })
map("n", "<S-l>", "<cmd>tabnext<CR>", { desc = "下一个 Tab" })
map("n", "<leader><Tab>n", "<cmd>tabnew<CR>", { desc = "新建 Tab" })
map("n", "<leader><Tab>c", "<cmd>confirm tabclose<CR>", { desc = "关闭当前 Tab" })
map("n", "<leader><Tab>o", "<cmd>confirm tabonly<CR>", { desc = "仅保留当前 Tab" })
map("n", "<leader><Tab>r", function()
  vim.ui.input({ prompt = "Tab 名称: " }, function(name)
    if name ~= nil then
      require("tabby.feature.tab_name").set(0, name)
    end
  end)
end, { desc = "重命名当前 Tab" })
map("n", "<leader><Tab>j", "<cmd>Tabby jump_to_tab<CR>", { desc = "跳转到 Tab" })
map("n", "<leader><Tab>w", "<cmd>Tabby pick_window<CR>", { desc = "选择 Window" })
map("n", "<leader><Tab>h", "<cmd>-tabmove<CR>", { desc = "向左移动当前 Tab" })
map("n", "<leader><Tab>l", "<cmd>+tabmove<CR>", { desc = "向右移动当前 Tab" })
